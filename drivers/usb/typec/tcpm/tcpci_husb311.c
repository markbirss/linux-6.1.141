...
static irqreturn_t husb311_irq(int irq, void *dev_id)
{
	...
	if (status & TCPC_ALERT_CC_STATUS) {
		...
		if (chip->sw_gpio) {
			if (cc1 == TYPEC_CC_RD || cc1 == TYPEC_CC_RP_DEF || cc1 == TYPEC_CC_RP_1_5 || cc1 == TYPEC_CC_RP_3_0) {
				gpiod_set_value(chip->sw_gpio, 1);
				dev_dbg(chip->dev, "CC1 detected, set sw-gpio HIGH\n");
			} else if (cc2 == TYPEC_CC_RD || cc2 == TYPEC_CC_RP_DEF || cc2 == TYPEC_CC_RP_1_5 || cc2 == TYPEC_CC_RP_3_0) {
				gpiod_set_value(chip->sw_gpio, 0);
				dev_dbg(chip->dev, "CC2 detected, set sw-gpio LOW\n");
			}
		}
			 ...
			 const struct i2c_device_id *i2c_id)
{
	...
	chip->sw_gpio = devm_gpiod_get_optional(chip->dev, "sw", GPIOD_OUT_LOW);
	if (IS_ERR(chip->sw_gpio)) {
		ret = PTR_ERR(chip->sw_gpio);
		chip->sw_gpio = NULL;
		if (ret != -ENODEV)
			return ret;
	}
