...
static ssize_t force_rescan_store(struct device *dev,
				...
				size_t count)
{
	...
	if (!host) {
		pr_warn("mmc: force_rescan: host is NULL\n");
		return -ENODEV;
	}
	...
	switch (val) {
	case 1: /* Full reset for normal SD/MMC cards */
		if (host->caps & MMC_CAP_SDIO_IRQ) {
			pr_warn("mmc: force_rescan: full power cycle not safe for SDIO, fallback to immediate rescan\n");
			host->rescan_entered = 0;
			mmc_rescan(&host->detect.work);
			break;
		}

		pr_info("mmc: force_rescan: full power cycle on host %s\n",mmc_hostname(host));
	...
	case 2: /* Detect change only */
		pr_info("mmc: force_rescan: immediate rescan on host %s\n",
		mmc_hostname(host));
		host->rescan_entered = 0;
		mmc_rescan(&host->detect.work);
		break;
...
static DEVICE_ATTR_WO(force_rescan);
...
int mmc_add_host(struct mmc_host *host)
{
	...
	device_create_file(&host->class_dev, &dev_attr_force_rescan);
...
void mmc_remove_host(struct mmc_host *host)
{
	...
	device_remove_file(&host->class_dev, &dev_attr_force_rescan);
