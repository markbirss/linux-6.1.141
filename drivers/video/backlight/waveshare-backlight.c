...
#define RASPI_REG_LCD		0x83
#define RASPI_REG_PWM		0x86
#define RASPI_CFG_BL_EN		BIT(0)
...
static int ws_set_brightness(struct waveshare_bl_data *data, u32 brightness)
{
	...
	if (g_panel_mode == RASPI_MODE) {
		...
		read_data = i2c_smbus_read_byte_data(data->i2c, RASPI_REG_LCD);
		if (read_data < 0) {
			return read_data;
		}

		if (read_data & RASPI_CFG_BL_EN) {
			if (val == 0) {
				write_data = read_data & ~RASPI_CFG_BL_EN;
				ret = i2c_smbus_write_byte_data(data->i2c, RASPI_REG_LCD, write_data);
				if (ret) {
		...
		ret = i2c_smbus_write_byte_data(data->i2c, RASPI_REG_PWM ,val);
		if (ret) {
			dev_err(data->dev, "I2C write failed: %d\n", ret);
			return ret;
		}
